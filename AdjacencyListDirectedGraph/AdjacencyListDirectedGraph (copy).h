...
public:
  ...
  public:
    ...
    bool isAdjacentTo(const Edge& edge) const {
        if(e_obj->origin_vertex == edge.dest() || e_obj->dest_vertex == edge.origin()){
          return true;
        }
      return false;
    }
...
public:
  ...
  Vertex insertVertex(const V& x) {
      ...
      this->vertex_collection.push_back(*temp);
  ...
  Edge insertDirectedEdge(const Vertex& v, const Vertex& w, E x) {
    ...
    for(auto it = vertex_collection.begin(); it != vertex_collection.end(); ++it){
      if((it->elt) == *v){
        *cur->origin_inc_edges_pos = *it->inc_edges_pos;
        v_it = it;
      }
      if(it == w)
        cur->dest_inc_edges_pos = it->dest_inc_edges_pos;
        w_it = it;
    }

    v_it->inc_edges_pos.push_back(Edge(cur));
    w_it->inc_edges_pos.push_back(Edge(cur));
    this->edge_collectin.push_back(*cur);
